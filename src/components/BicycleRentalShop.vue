<template>
  <div>
    ...
    <p>Number of bicycles: {{ $store.state.count }}</p>
    ...
    <p v-if="$store.state.count == 0" v-bind:style="{ color: textdanger }">No bike is available</p>
    <p v-else v-bind:style="{ color: textsuccess }">Bikes are available</p>
    <hr>
    <h2>Rules</h2>
    <ol>
      <li v-for="(rule, index) in $store.state.rules" v-bind:key="index">{{ rule }}</li>
    </ol>
    <hr>
